...
current_version="$($container_cli inspect --format '{{index .Config.Labels "pi.version"}}' "$image:latest" 2>/dev/null || echo "")"
...
$container_cli build $cache_flag \
    --build-arg "BASE_IMAGE=$base_image" \
    --build-arg "NPM_EXTRA_CA_CERT=${NPM_EXTRA_CA_CERT:-}" \
